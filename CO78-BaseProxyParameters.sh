...
read -rp "Foreman Hostname to use: ($ProxyHOSTNAME): " choice;[[ -n "${choice}"  ]] &&  export ProxyHOSTNAME="$choice";
read -rp "Foreman IP to use: ($ProxyIP): " choice; [[ -n "${choice}"  ]] &&  export ProxyIP="$choice";
read -rp "Foreman Netmask to use (just the number of bits): ($ProxyNETMASK): " choice;	[[ -n "${choice}"  ]] &&  export ProxyNETMASK="$choice";
read -rp "Foreman GW to use: ($ProxyGW): " choice; [[ -n "${choice}"  ]] &&  export ProxyGW="$choice";
read -rp "Foreman Global DNS to use: ($ProxyDNSSERVER): " choice;	[[ -n "${choice}"  ]] &&  export ProxyDNSSERVER="$choice";
read -rp "Foreman Password to use: ($ProxyPass): " choice;	[[ -n "${choice}"  ]] &&  export ForemanPass="$choice";
...
echo "export ProxyDNSSERVER=$ProxyDNSSERVER" >> ~/proxysetup/variables.sh
echo "export ForemanPass=$ForemanPass" >> ~/proxysetup/variables.sh
